...
struct Pincel{
    ...
    Pincel(sf::RenderWindow& window): window(window){
        if(!font.loadFromFile("C:/Windows/Fonts/JOKERMAN.TTF")){
...
struct Board{
    ...
    void draw(){
        pencil.write("Hite: " + std::to_string(this->hits) + " Misses: " + std::to_string(this->misses), 10, 10, 20, sf::Color::White);
...
struct Game{
    ...
    void gameover(){
        ...
        window.clear(sf::Color::Black);
        pincel.write("Game Over", 250, 250, 50, sf::Color::Red);
        window.display();
    }
